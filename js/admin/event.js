$(function() {
  ...
  var uploadProgress = new window.mdc.linearProgress.MDCLinearProgress(
    document.getElementById("image-upload-progress")
  );
  var deleteDialog = new window.mdc.dialog.MDCDialog(
    document.getElementById("delete-dialog")
  );
  var updateDialog = new window.mdc.dialog.MDCDialog(
  ...
  var imageFileUpload = document.querySelector(
    "#eventImagePreview .image-file-upload"
  );
  var imagePreview = document.getElementById("eventImagePreview");
  var imagePreviewRipple = new window.mdc.ripple.MDCRipple(
  ...
  function validateForm() {
    var optionalFields = dataMap.map(function(datumMap) {
      return datumMap[3];
    });
    var inputFields = dataMap
      .filter(function(datumMap) {
        return !datumMap[3];
      })
      .map(function(datumMap) {
        return datumMap[0];
      });
    var valueAttributes = dataMap.map(function(datumMap) {
      return datumMap[2];
    });
    var valid = true;
  ...
  function resetForm() {
    var inputFields = dataMap.map(function(datumMap) {
      return datumMap[0];
    });
    var valueAttributes = dataMap.map(function(datumMap) {
      return datumMap[2];
  ...
  function fillForm(docId) {
    var data = fetchedData.find(function(datum) {
      return datum.id === docId;
    });
    if (!data) return;
    dataMap.forEach(function(datumMap) {
      ...
      if (data[key]) {
        if (valueAttribue) {
          $(selector)[0][valueAttribue] =
            data[key] && data[key].seconds
              ? new Date(data[key].seconds * 1000)
              : data[key];
        } else {
  ...
  function uploadImage(eventId, imageFile) {
    ...
    uploadTask.on("state_changed", function(snapshot) {
      ...
      $(".image-upload-progress-wrapper")
        .find(".image-upload-progress-text")
        .text(progress * 100 + "%");
    });
    return uploadTask;
  }
    ...
    db.collection("events")
      ...
      .then(function(docRef) {
        ...
        if (imageFileUpload.files && imageFileUpload.files[0]) {
          return uploadImage(eventId, imageFileUpload.files[0]).then(
            function() {
              finish();
            }
          );
    ...
    db.collection("events")
        ...
        if (querySnapshot.size === 0) {
          if ((querySnapshot.metadata.fromCache = true)) {
            ...
            $eventList.html(
              template.error({
                error:
                  "There are no existing events. Create one with the button by the corner."
              })
            );
          }
          return;
        ...
        return Promise.all(
          ...
          data = data.map(function(datum) {
            return Object.assign({}, datum, {
              date: datum.date
                ? new Date(datum.date.seconds * 1000).toLocaleDateString()
                : "",
              time: datum.time
                ? new Date(datum.time.seconds * 1000).toLocaleTimeString()
                : ""
            });
          });
          data = data.map(function(datum) {
          ...
          $eventList.find(".mdc-menu").each(function() {
            ...
            var $menuBtn = $eventList.find(
              '.menu-button[data-event-id="' + eventId + '"]'
            );
      ...
      .catch(function(err) {
        ...
        $eventList.html(
          template.error({
            error:
              "Failed to fetch events, please check your internet connection"
          })
        );
      });
  }
